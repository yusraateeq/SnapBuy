...
const SmoothSlider = () => {
  ...
  return (
    <div className="relative w-full py-20">
      <h2 className="text-4xl md:text-6xl text-pink-600 font-extrabold text-center mb-6">
      ...
      <div className="relative flex items-center justify-center">
        ...
        <button
          ...
          className={`absolute left-4 bg-white p-2 rounded-full shadow-lg ${currentIndex === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
        >
        ...
        <div className="overflow-hidden w-full sm:w-11/12">
          ...
          >
            {products.map((product, index) => (
              <div
                key={`${product.id}-${index}`} // Use both id and index to make the key unique
                className="min-w-full sm:min-w-[45%] md:min-w-[25%] p-4 bg-white rounded-lg shadow-md mx-2"
                data-aos="fade-up" // Add AOS animation
                data-aos-duration="1000" // Adjust the animation duration
                ...
                <div className="flex justify-center items-center mt-2 text-yellow-500">
                  {Array.from({ length: 5 }, (_, i) => (
                    <svg
                      ...
                      fill={i < product.rate ? "currentColor" : "none"}
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      className="w-4 h-4"
                    >
        ...
        <button
          ...
          className={`absolute right-4 bg-white p-2 rounded-full shadow-lg ${currentIndex >= products.length - 4 ? "opacity-50 cursor-not-allowed" : ""}`}
        >
